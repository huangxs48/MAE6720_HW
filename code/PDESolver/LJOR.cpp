...
void PDESolver::lineJacobi(SteadyHeatConduction2d *peq, Mesh *pmesh, 
								                MatrixSolver *pms, array2d &u_in, array2d &u_out, double omega)
{
  ...
  for (int j=js; j<=je; j++){
    ...
    //write into the next step array
    u_out.push_back(x);

  }
  //add y boundaries
  std::vector<double> x2_inner = u_in.at(jl);
  std::vector<double> x2_outter = u_in.at(ju);

  u_out.insert(u_out.begin(), x2_inner);
  u_out.push_back(x2_outter);

  pbval->Apply2DBval(pmesh, u_out);
  peq->Residual = res/(N);
